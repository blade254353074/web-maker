...
!function() {
    ...
    template("articleDetail", function($data) {
        "use strict";
        var $utils = this, $string = ($utils.$helpers, $utils.$string), id = $data.id, title = $data.title, avatar = $data.avatar, author = $data.author, time = $data.time, banner = $data.banner, content = $data.content, like = $data.like, $out = "";
        return $out += '<div class="article-detail" data-id="', $out += $string(id), $out += '"> <div class="paper reader-text"> <div class="article-header"> <h3 class="title">', 
        $out += $string(title), $out += '</h3> <div class="meta"> <a href="javascript:;"><img src="', 
        $out += $string(avatar), $out += '" class="avatar"></a> <div class="meta-main"> <p>', 
        $out += $string(author), $out += "</p> <p>", $out += $string(time), $out += '</p> </div> </div> <div class="thumbnail"> <img src="', 
        $out += $string(banner), $out += '"> </div> </div> <div class="article-body">', 
        $out += $string(content), $out += '</div> </div> <div class="artcle-tool"> ', $out += like ? ' <a class="tool-like active"> ' : ' <a class="tool-like"> ', 
    ...
    template("articleList", function($data) {
        "use strict";
        var $utils = this, $helpers = $utils.$helpers, $each = $utils.$each, list = $data.list, $string = ($data.$value, 
        $data.$index, $utils.$string), total = $data.total, page = $data.page, cate = $data.cate, $out = "";
        return $out += "<ul> ", $each(list, function($value) {
            $out += ' <li class="article-img"> <a class="wrap-img" href="/#/article/detail/', 
            $out += $string($value.id), $out += '"> ', $value.cover ? ($out += ' <div class="bg" style="background-image:url(', 
            $out += $string($value.cover), $out += ')"></div> ') : $out += ' <div class="bg" style="background-image:url(/assets/imgs/ph_square.png)"></div> ', 
            $out += ' </a> <div class="wrap-text"> <p class="list-top"> <a href="" class="author-name">', 
            $out += $string($value.author), $out += '</a> <em>·</em> <span class="time" title="', 
            $out += $string($value.time), $out += '">', $out += $string($helpers.dateDiff($value.time)), 
            $out += '</span> </p> <h4 class="title"><a href="/#/article/detail/', $out += $string($value.id), 
            $out += '">', $out += $string($value.title), $out += '</a></h4> <div class="list-footer"> <a href="/#/article/detail/', 
            $out += $string($value.id), $out += '">阅读 ', $out += $string($value.read), $out += '</a> <em>·</em> <a href="/#/article/detail/', 
            $out += $string($value.id), $out += '">喜欢 ', $out += $string($value.like), $out += "</a> </div> </div> </li> ";
    ...
    template("product", function($data) {
        "use strict";
        var $utils = this, $helpers = $utils.$helpers, $each = $utils.$each, list = $data.list, $string = ($data.$value, 
        $data.$index, $utils.$string), total = $data.total, page = $data.page, $out = "";
        ...
        $each(list, function($value) {
            ...
            $out += $string($value.id), $out += '"> <div class="bg" style="background-image: url(', 
            $out += $string($value.cover), $out += ');"></div> <div class="content"> <h4 class="title">', 
    ...
    template("productDetail", function($data) {
        "use strict";
        var $utils = this, $string = ($utils.$helpers, $utils.$string), cover = $data.cover, title = $data.title, summary = $data.summary, content = $data.content, $out = "";
        return $out += '<div class="product-article reader-text"> <div class="paper"> <div class="content"> <div class="close-wrapper"> <a id="close" class="close"></a> </div> <div class="paper-header" style="background-image: url(', 
        $out += $string(cover), $out += ')"> <div class="bg"></div> <div class="text"> <h3 class="title">', 
        $out += $string(title), $out += '</h3> <p class="summary">', $out += $string(summary), 
        $out += '</p> </div> </div> <div class="paper-body"> <article> ', $out += $string(content), 
